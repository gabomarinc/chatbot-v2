...
import { ProspectKanbanModal } from './ProspectKanbanModal'
...
function ProspectCard({
    prospect, customFields, columnColor,
    onDragStart, onCardClick, isDragging
}: {
    prospect: Prospect
    customFields: CustomField[]
    columnColor: string
    ...
    return (
        <div
            ...
            className={cn(
                'bg-white rounded-2xl border border-gray-100 shadow-sm p-4 cursor-pointer hover:shadow-md transition-all group select-none',
        ...
        >
            ...
            <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                    <h4 className="font-black text-[13px] text-gray-900 truncate">{prospect.name}</h4>
                    {prospect.agentName && (
                ...
                <GripVertical className="w-3.5 h-3.5 text-gray-200 group-hover:text-gray-400 shrink-0 mt-0.5 cursor-grab active:cursor-grabbing" />
            </div>
            ...
            {relevantFields.length > 0 && (
                <div className="space-y-1 pt-2 border-t border-gray-50 mb-2">
                    {relevantFields.slice(0, 2).map(f => (
                        <div key={f.key} className="flex items-center justify-between gap-2">
                            <span className="text-[9px] font-black uppercase tracking-widest text-gray-300 truncate">{f.label}</span>
                            <span className="text-[10px] font-bold text-gray-600 truncate max-w-[55%] text-right">
                                {String(prospect.customData[f.key])}
                            </span>
                        </div>
                    ))}
                    {relevantFields.length > 2 && (
                        <p className="text-[9px] text-gray-300 text-right">+{relevantFields.length - 2} campos más</p>
                    )}
                </div>
            )}
            ...
            {prospect.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 pt-2 border-t border-gray-50">
                    {prospect.tags.slice(0, 2).map((tag, i) => (
                        <span key={i} className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-gray-100 text-gray-500 text-[9px] font-bold rounded-full">
            ...
            {/* Hover hint */}
            <div className="mt-2 pt-2 border-t border-gray-50 text-right opacity-0 group-hover:opacity-100 transition-opacity">
                <span className="text-[9px] font-black uppercase tracking-widest" style={{ color: columnColor }}>
                    Ver detalle →
                </span>
            </div>
        </div>
        ...
        <div className="flex flex-col min-w-[280px] max-w-[300px] w-full flex-shrink-0">
            <div className="flex items-center justify-between mb-3 px-1">
                <div className="flex items-center gap-2">
                    ...
                    <span className="min-w-[20px] h-5 px-1.5 rounded-full bg-gray-100 flex items-center justify-center text-[10px] font-black text-gray-500">
                        {prospects.length}
                    </span>
                </div>
                {!column.isDefault && (
            ...
            <div
                ...
                className={cn(
                    ...
                    isOver ? 'border-2 border-dashed ring-2 ring-offset-1 bg-gray-50' : 'bg-gray-50/70 border border-gray-100'
                )}
            ...
            >
                ...
                {prospects.length === 0 && !isOver && (
                    <div className="flex flex-col items-center justify-center h-32 text-gray-200">
                        ...
                        <p className="text-[10px] font-bold">Sin prospectos</p>
                    </div>
                )}
                {isOver && (
                    <div className="h-14 rounded-xl border-2 border-dashed flex items-center justify-center" style={{ borderColor: column.color }}>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
            <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm p-6">
                <div className="flex items-center justify-between mb-5">
                    <h3 className="font-black text-gray-900">Nueva Columna</h3>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Ej: Calificado, Cerrado..."
                ...
                <div className="flex gap-2 mt-6">
                    ...
                    <button onClick={handleCreate} disabled={!name.trim() || loading}
                        className="flex-1 py-2.5 rounded-2xl bg-[#21AC96] text-white font-bold text-sm hover:bg-[#1b8c7a] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                    >
...
/* ─── Main Board ─────────────────────────────────────────────────────── */
export function ProspectsKanbanBoard({ initialColumns, initialProspects, agents, customFields }: ProspectsKanbanBoardProps) {
    ...
    const handleDrop = useCallback(async (e: React.DragEvent, targetColumn: string) => {
        ...
        if (!prospect || prospect.prospectStatus === targetColumn) { setDraggingId(null); return }
    ...
    const handleCreateColumn = async (name: string, color: string) => {
        ...
        if (res.success && res.column) { setColumns(prev => [...prev, res.column as Column]); toast.success(`Columna "${name}" creada`) }
        else toast.error('Error al crear columna')
    }

    const handleDeleteColumn = async (id: string) => {
        const col = columns.find(c => c.id === id)
        if (!col || !confirm(`¿Eliminar "${col.name}"? Los prospectos irán a "Nuevo".`)) return
        ...
        } else toast.error('Error al eliminar columna')
    }

    // Get column color for modal accent
    const getColumnColor = (status: string) =>
        columns.find(c => c.name === status)?.color ?? '#21AC96'

    return (
        <div className="flex flex-col h-full">
            {/* Toolbar */}
            <div className="flex flex-wrap items-center gap-3 mb-6">
                <div className="flex items-center gap-1 bg-white rounded-2xl border border-gray-100 shadow-sm p-1 flex-wrap">
                    <button onClick={() => setSelectedAgent('all')}
                        className={cn('px-4 py-2 rounded-xl text-[12px] font-black transition-all', selectedAgent === 'all' ? 'bg-[#21AC96] text-white shadow-sm' : 'text-gray-500 hover:text-gray-800')}
                    >Todos</button>
                    {agents.map(a => (
                        <button key={a.id} onClick={() => setSelectedAgent(a.id)}
                            className={cn('px-4 py-2 rounded-xl text-[12px] font-black transition-all', selectedAgent === a.id ? 'bg-[#21AC96] text-white shadow-sm' : 'text-gray-500 hover:text-gray-800')}
                        >{a.name}</button>
                    ))}
                </div>
                ...
                <div className="flex items-center gap-2 ml-auto">
                    ...
                    <span className="text-[12px] text-gray-400 font-bold">{filteredProspects.length} prospectos</span>
                    <button onClick={() => setShowNewColumnModal(true)}
                        className="flex items-center gap-1.5 px-4 py-2 bg-white border border-gray-100 rounded-2xl text-[12px] font-black text-gray-500 hover:text-[#21AC96] hover:border-[#21AC96]/30 transition-all shadow-sm"
                    >
            ...
            <div className="flex gap-4 overflow-x-auto pb-6 flex-1" onDragEnd={() => setDraggingId(null)}>
                ...
                <button onClick={() => setShowNewColumnModal(true)}
                    className="flex-shrink-0 w-[180px] min-h-[180px] rounded-[1.25rem] border-2 border-dashed border-gray-200 flex flex-col items-center justify-center gap-2 text-gray-300 hover:border-[#21AC96]/40 hover:text-[#21AC96]/60 transition-all group"
                >
                    <Plus className="w-6 h-6 group-hover:scale-110 transition-transform" />
                    <span className="text-[10px] font-black">Agregar columna</span>
                </button>
            </div>
            ...
            {modalProspect && (
                <ProspectKanbanModal
                    ...
                    customFields={customFields}
                    statusColor={getColumnColor(modalProspect.prospectStatus)}
