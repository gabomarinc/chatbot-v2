...
function AgentCard({ agent }: { agent: Agent }) {
    ...
    const handleDelete = async (e: React.MouseEvent) => {
        ...
        if (!confirm('¿Estás seguro de eliminar este agente? Esta acción es irreversible.')) return;

        try {
            const { deleteAgent } = await import('@/lib/actions/dashboard');
            await deleteAgent(agent.id);
            toast.success('Agente eliminado');
            // Optimistic update or refresh is handled by startTransition usually, 
            // but here we might rely on router.refresh() from the server action or manual refresh
            router.refresh();
        } catch (error) {
